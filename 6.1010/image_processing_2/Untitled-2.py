...
def lowest_cost(recipes_db, food_name, forbidden_items=None):
    ...
    atomic_costs = {
        food: cost for food, cost in atomic_ingredient_costs(recipes_db).items()
    ...
    for food, recipes in compound_ingredient_possibilities(recipes_db).items():
        if food not in forbidden_items: # add to "valid" list if food itself not forbidden
            valid_recipes = []
        ...
        # recursive case: food is compound
            ...
            for variant_recipe in compound_possibilities[food]:
                ...
                for ingredient, quantity in variant_recipe:
                    ...
                    if ingredient not in atomic_costs and ingredient not in compound_possibilities:
                        check = False
                        break
...
def cheapest_flat_recipe(recipes_db, food_name, forbidden_items=None):
    ...
    # helper function that calculates the cost of a recipe
            ...
            for variant_recipe in compound_possibilities[food]:
                ...
                for ingredient, quantity in variant_recipe:
                    ...
                    if ingredient_recipe is None:
                        ...
                        break
                    # scale recipe
                    scaled_ingredient_recipe = scaled_recipe(ingredient_recipe, quantity)
                    # get full recipe
                ...
                if check:
                    ...
                    if total_cost is not None and (min_cost is None or total_cost < min_cost):
    ...
    def recursive_combinations(index, current_recipe):
        ...
        if index == len(nested_recipes):
            ...
            for recipe in nested_recipes[index]:
                new_recipe = add_recipes([current_recipe, recipe]) # call helper function
            ...
            for variant_recipe in compound_possibilities[food]:
                ...
                for ingredient, quantity in variant_recipe:
                    ...
                    if not ingredient_recipes:
                        ...
                        break
                    # scaling recipes
                    scaled_recipes = [scaled_recipe(var, quantity) for var in ingredient_recipes]
